...
- **Federation** — [Apollo Federation](https://www.apollographql.com/docs/federation/) (or compatible) composition of multiple GraphQL subgraphs behind a **gateway** (e.g. Apollo Router, [GraphQL Hive](https://the-guild.dev/graphql/hive) as a schema registry / gateway stack).
...
| **Federation- and gateway-ready subgraphs**           | Many deployments expose Gene-built schemas as **subgraphs** behind a **federated gateway** (Apollo Federation) or a **managed graph / gateway** path (e.g. Hive with router). v2 must **confirm** that emitted SDL and resolver wiring work in that model—not only as a standalone server. **Evidence:** **integration tests** that compose at least one Gene subgraph with a minimal second subgraph (or gateway fixture) and assert successful query execution through the gateway (see §6). Scope may narrow later (e.g. Federation 2 only), but the plan treats “works as a subgraph” as a first-class release criterion.                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
- **Federation and federated gateways (Apollo, Hive, etc.):** Before v2 ships, **confirm** that a Gene-generated schema can run as a **subgraph** in a **federated** setup: correct use of federation directives where needed (`@key`, `@shareable`, `@extends`, etc.—exact set TBD by subgraph design), no SDL or runtime behavior that breaks composition, and resolvers that behave correctly when the subgraph is invoked through a **gateway** (not only when the Gene app owns the entire root `Query` / `Mutation`). Treat **GraphQL Hive** (or an equivalent registry + router path) as the same class of requirement: if the product standard is “subgraph + gateway,” the library should be validated on that path, not only on a monolith server. **Integration tests** are the source of truth: add automated tests that build a small Gene subgraph, compose it with a **minimal partner subgraph** (or a test harness that runs the official composition / gateway stack in CI), and execute **end-to-end queries** through the gateway against the Gene subgraph’s types. Document any **known limitations** (e.g. unsupported directive combinations) next to the tests.
...
1. **Design lock**: List wrapper + **deep filtering** (§2.7); **polymorphic unions / page blocks** (§2.8); pagination (§3); CMS + **§4.6 nav/discovery** + **`registerCmsModel` scope** + translations (§4); **§5.4–§5.8** (`roles` grammar, actions, path gate, mutation ordering, **optional (recommended) TS role augmentation**); **Markdown docs** (§11); **federation / gateway subgraph contract** (§6)—including which federation version and gateway stack CI will target.
2. **Schema generation**: New types (including **union** types and hub fields per §2.8); optional deprecation flag for v1-shaped lists if needed on v1.x. **Federation / gateway:** land **integration tests** early so composition and gateway-routed queries stay green as schema generation evolves (§6).
...
- **Federation / gateway (§6):** Exact **Federation spec version** (e.g. v2), whether CI runs **Apollo Router**, **Hive’s router path**, or both, and how minimal the **partner subgraph** fixture should be vs. full gateway binaries in CI.
