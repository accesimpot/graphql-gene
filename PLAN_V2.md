...
| **Federation- and gateway-ready subgraphs**           | Gene-built schemas should work as **subgraphs** behind **Apollo Federation** and typical **gateway / managed graph** setups (e.g. **Hive**). Confirm with **integration tests**; scenario detail can stay light until implementation (see §6).                                                                                                                                                                                                                                                                                    |
...
- **Federation and gateways (Apollo, Hive, etc.):** Support “subgraph behind a federated or managed gateway” only once **integration tests** cover composition and a query through the gateway. Keep scenarios minimal; note material limits next to the tests if needed.
...
1. **Design lock**: List wrapper + **deep filtering** (§2.7); **polymorphic unions / page blocks** (§2.8); pagination (§3); CMS + **§4.6 nav/discovery** + **`registerCmsModel` scope** + translations (§4); **§5.4–§5.8** (`roles` grammar, actions, path gate, mutation ordering, **optional (recommended) TS role augmentation**); **Markdown docs** (§11); **federation / gateway** confirmation via integration tests (§6).
2. **Schema generation**: New types (including **union** types and hub fields per §2.8); optional deprecation flag for v1-shaped lists if needed on v1.x. Add federation / gateway **integration tests** early (§6).
...
- **Federation / gateway (§6):** Federation version, CI toolchain (e.g. Router vs Hive path), and how heavy the test harness is.
