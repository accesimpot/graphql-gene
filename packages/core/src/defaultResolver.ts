...
export type GenerateDefaultQueryFilterTypeDefsOptions = {
  ...
  /**
   * Resolve the underlying target GraphQL type of a multi-association list wrapper (e.g.
   * `{ count, items: [Product!]! }` → `Product`), or `undefined` when `graphqlTypeName` is not a
   * wrapper.
   *
   * Multi (list) associations expose a wrapper object type instead of the target type. The wrapper
   * itself is not filterable, so it must never be used to derive a `where` input. When provided,
   * nested multi-association fields reference the association's canonical where input (generated by
   * that association's own parent-level filter hook) rather than deriving a bogus input from the
   * wrapper's `count`/`items` fields.
   */
  resolveListWrapperTargetType?: (graphqlTypeName: string) => string | undefined
}
  ...
  Object.entries(options.typeDefLines[options.fieldType].lines).forEach(
    ([returnFieldKey, returnFieldConfig]) => {
      ...
      if (validInputType) {
        ...
        const wrapperTargetType = options.resolveListWrapperTargetType?.(relatedGraphqlType)

        if (wrapperTargetType) {
          // Multi (list) association: `relatedGraphqlType` is a `{ count, items }` wrapper, not a
          // filterable entity. Its canonical where input (`nestedWhereInputName`) is generated from
          // the association target by that association's own parent-level filter hook, so reference
          // it here instead of deriving a bogus input from the wrapper's own fields.
          whereTypeDef = nestedWhereInputName
        } else {
          // Singular association: no parent-level hook generates this input, so build it from the
          // target type (scalars only, deeper associations dropped at depth 0).
          generateDefaultQueryFilterTypeDefs({
            ...
            resolveListWrapperTargetType: options.resolveListWrapperTargetType,
          })
